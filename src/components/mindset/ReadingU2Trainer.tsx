...
import {
  ...
  MS_U2_READ_NEXT,
  readingU2,
} from "../../data/mindset/readingU2";

const data = readingU2;
...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, MS_U2_READ_STEPS.length - 1));
}
...
export function ReadingU2Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= MS_U2_READ_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const nextLabel =
    needsCheck && !checked ? "Check →" : MS_U2_READ_NEXT[step];
    ...
    <div className="app-shell reading-flow reading-flow--viewport read-m3">
      ...
      >
        ...
        <span className="flow-footer__step">
          {step + 1} / {MS_U2_READ_STEPS.length}
        </span>
