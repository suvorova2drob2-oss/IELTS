...
import {
  ...
  MS_U2_SPEAK_NEXT,
  speakingU2,
} from "../../data/mindset/speakingU2";

const data = speakingU2;
...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, MS_U2_SPEAK_STEPS.length - 1));
}
...
export function SpeakingU2Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= MS_U2_SPEAK_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const nextLabel =
    needsCheck && !checked ? "Check →" : MS_U2_SPEAK_NEXT[step];
    ...
    <div className="app-shell reading-flow reading-flow--viewport speak-m3a">
      ...
      >
        ...
        <span className="flow-footer__step">
          {step + 1} / {MS_U2_SPEAK_STEPS.length}
        </span>
