...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, MS_U3_LISTEN_STEPS.length - 1));
}
...
export function ListeningU3Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= MS_U3_LISTEN_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const nextLabel =
    needsCheck && !checked ? "Check →" : MS_U3_LISTEN_NEXT[step];
    ...
    <div className="app-shell reading-flow reading-flow--viewport listen-m3b">
      <div className="reading-chrome">
        ...
        <div className="learn-step-tabs">
          {MS_U3_LISTEN_STEPS.map((label, i) => (
            <button
              key={label}
              type="button"
      ...
      >
        ...
        <span className="flow-footer__step">
          {step + 1} / {MS_U3_LISTEN_STEPS.length}
        </span>
