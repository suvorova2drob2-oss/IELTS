...
import {
  ...
  MS_U4_LISTEN_NEXT,
  listeningU4,
} from "../../data/mindset/listeningU4";

const data = listeningU4;
...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, MS_U4_LISTEN_STEPS.length - 1));
}
...
export function ListeningU4Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= MS_U4_LISTEN_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const nextLabel =
    needsCheck && !checked ? "Check →" : MS_U4_LISTEN_NEXT[step];
    ...
    <div className="app-shell reading-flow reading-flow--viewport listen-m3b">
      <div className="reading-chrome">
        ...
        <div className="learn-step-tabs">
          {MS_U4_LISTEN_STEPS.map((label, i) => (
            <button
              key={label}
              type="button"
      ...
      >
        ...
        <span className="flow-footer__step">
          {step + 1} / {MS_U4_LISTEN_STEPS.length}
        </span>
