...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, MS_U4_WRITE_STEPS.length - 1));
}
...
export function WritingU4Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const needsCheck =
    step < MS_U4_WRITE_STEPS.length - 1 &&
    curKind !== "exam" &&
    curKind !== "intro";
  ...
  const goNext = () => {
    ...
    if (step >= MS_U4_WRITE_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const nextLabel =
    needsCheck && !checked ? "Check →" : MS_U4_WRITE_NEXT[step];
  ...
  return (
    <div className="app-shell reading-flow reading-flow--viewport write-m2a">
      ...
      <footer className={`flow-footer ${checked && needsCheck ? "flow-footer--checked" : ""}`}>
        ...
        <span className="flow-footer__step">{step + 1} / {MS_U4_WRITE_STEPS.length}</span>
