...
import {
  ...
  MS_U4_SPEAK_NEXT,
  speakingU4,
} from "../../data/mindset/speakingU4";

const data = speakingU4;
...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, MS_U4_SPEAK_STEPS.length - 1));
}
...
export function SpeakingU4Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= MS_U4_SPEAK_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const nextLabel =
    needsCheck && !checked ? "Check →" : MS_U4_SPEAK_NEXT[step];
    ...
    <div className="app-shell reading-flow reading-flow--viewport speak-m3a">
      ...
      >
        ...
        <span className="flow-footer__step">
          {step + 1} / {MS_U4_SPEAK_STEPS.length}
        </span>
