...
import {
  ...
  MS_U3_SPEAK_NEXT,
  speakingU3,
} from "../../data/mindset/speakingU3";

const data = speakingU3;
...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, MS_U3_SPEAK_STEPS.length - 1));
}
...
export function SpeakingU3Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= MS_U3_SPEAK_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const nextLabel =
    needsCheck && !checked ? "Check →" : MS_U3_SPEAK_NEXT[step];
    ...
    <div className="app-shell reading-flow reading-flow--viewport speak-m3a">
      ...
      >
        ...
        <span className="flow-footer__step">
          {step + 1} / {MS_U3_SPEAK_STEPS.length}
        </span>
