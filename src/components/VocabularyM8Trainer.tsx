...
import {
  ...
  VOCAB_M8_STEPS,
  vocabularyM8,
} from "../data/vocabularyM8";

const data = vocabularyM8;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, VOCAB_M8_STEPS.length - 1));
}

export function VocabularyM8Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= VOCAB_M8_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
