...
import {
  ...
  READ_M10A_STEPS,
  readingM10a,
} from "../data/readingM10a";

const data = readingM10a;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, READ_M10A_STEPS.length - 1));
}

export function ReadingM10aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const examScore = allExamGaps.filter((g) =>
    checkReadM10a(gaps[g.id] ?? "", g.answers),
  ).length;
  ...
  const goNext = () => {
    ...
    if (step >= READ_M10A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const gapRow = (
    ...
    const val = gaps[g.id] ?? "";
    const ok = checkReadM10a(val, g.answers);
    return (
      <li key={g.id}>
        <span>
          {g.id}. {g.before}
