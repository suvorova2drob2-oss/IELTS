...
import {
  ...
  SPEAK_M8B_STEPS,
  speakingM8b,
} from "../data/speakingM8b";

const data = speakingM8b;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, SPEAK_M8B_STEPS.length - 1));
}

export function SpeakingM8bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= SPEAK_M8B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
