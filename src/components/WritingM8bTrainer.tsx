...
import {
  ...
  WRITE_M8B_STEPS,
  writingM8b,
} from "../data/writingM8b";
import { WordCountMeter, countWords } from "./WordCountMeter";

const data = writingM8b;
const DRAFT_KEY = "ielts-writing-m8b-draft";
const PLAN_KEY = "ielts-writing-m8b-plan";

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, WRITE_M8B_STEPS.length - 1));
}
...
export function WritingM8bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= WRITE_M8B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
