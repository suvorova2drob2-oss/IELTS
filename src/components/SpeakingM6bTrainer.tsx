...
import {
  ...
  SPEAK_M6B_STEPS,
  speakingM6b,
} from "../data/speakingM6b";

const data = speakingM6b;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, SPEAK_M6B_STEPS.length - 1));
}

export function SpeakingM6bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= SPEAK_M6B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
