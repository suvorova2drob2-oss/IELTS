...
import {
  ...
  LISTEN_M9A_STEPS,
  listeningM9a,
} from "../data/listeningM9a";
import { ExpertDiscussPanel } from "./ExpertDiscussPanel";

const data = listeningM9a;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, LISTEN_M9A_STEPS.length - 1));
}

export function ListeningM9aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LISTEN_M9A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
        ...
        <section className="read-m3__panel">
          ...
          <ul className="read-m3__para-slots">
            {data.prep3.gaps3b.gaps.map((g) => {
              const val = gaps[g.id] ?? "";
              const ok = checkListenM9a(val, g.answers);
              return (
                <li key={g.id}>
                  {g.id}. {g.before}
                  <button
