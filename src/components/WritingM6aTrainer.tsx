...
const PLAN_KEY = "ielts-writing-m6a-plan";

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, WRITE_M6A_STEPS.length - 1));
}
...
export function WritingM6aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= WRITE_M6A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
