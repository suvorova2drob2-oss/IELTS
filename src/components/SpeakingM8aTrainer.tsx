...
import {
  ...
  SPEAK_M8A_STEPS,
  speakingM8a,
} from "../data/speakingM8a";
import { ExpertDiscussPanel } from "./ExpertDiscussPanel";

const data = speakingM8a;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, SPEAK_M8A_STEPS.length - 1));
}

export function SpeakingM8aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= SPEAK_M8A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
