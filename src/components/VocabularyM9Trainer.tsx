...
import {
  ...
  VOCAB_M9_STEPS,
  vocabularyM9,
} from "../data/vocabularyM9";

const data = vocabularyM9;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, VOCAB_M9_STEPS.length - 1));
}

export function VocabularyM9Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= VOCAB_M9_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
