...
const PLAN_KEY = "ielts-writing-M10b-plan";

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, WRITE_M10B_STEPS.length - 1));
}
...
export function WritingM10bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= WRITE_M10B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
            ...
            {data.forAgainst2a.items.map((it) => {
              ...
              return (
                <li key={it.id}>
                  ...
                  {(["for", "against", "both"] as const).map((v) => {
                    const on = val === v;
                    let cls = "";
                    if (checked) {
