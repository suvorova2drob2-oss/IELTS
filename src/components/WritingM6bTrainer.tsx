...
import {
  ...
  WRITE_M6B_STEPS,
  writingM6b,
} from "../data/writingM6b";
import { WritingComposePanel } from "./WritingComposePanel";

const data = writingM6b;
const DRAFT_KEY = "ielts-writing-m6b-draft";
const PLAN_KEY = "ielts-writing-m6b-plan";

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, WRITE_M6B_STEPS.length - 1));
}
...
export function WritingM6bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= WRITE_M6B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
