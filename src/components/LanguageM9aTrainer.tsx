...
import {
  ...
  LANG_M9A_STEPS,
  languageM9a,
} from "../data/languageM9a";

const data = languageM9a;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, LANG_M9A_STEPS.length - 1));
}

export function LanguageM9aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LANG_M9A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
