...
import {
  ...
  SPEAK_M10B_STEPS,
  speakingM10b,
} from "../data/speakingM10b";

const data = speakingM10b;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, SPEAK_M10B_STEPS.length - 1));
}

export function SpeakingM10bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= SPEAK_M10B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
