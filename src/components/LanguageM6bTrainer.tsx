...
import {
  ...
  LANG_M6B_STEPS,
  languageM6b,
} from "../data/languageM6b";

const data = languageM6b;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, LANG_M6B_STEPS.length - 1));
}

export function LanguageM6bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LANG_M6B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
