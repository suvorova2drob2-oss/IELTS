...
import {
  ...
  LANG_M8B_STEPS,
  languageM8b,
} from "../data/languageM8b";

const data = languageM8b;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, LANG_M8B_STEPS.length - 1));
}

export function LanguageM8bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LANG_M8B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
