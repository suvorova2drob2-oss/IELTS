...
import {
  ...
  READ_M6A_STEPS,
  readingM6a,
} from "../data/readingM6a";

const data = readingM6a;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, READ_M6A_STEPS.length - 1));
}

export function ReadingM6aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const examScore = allExamGaps.filter((g) =>
    checkReadM6a(gaps[g.id] ?? "", g.answers),
  ).length;
  ...
  const goNext = () => {
    ...
    if (step >= READ_M6A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const gapRow = (
    ...
    const val = gaps[g.id] ?? "";
    const ok = checkReadM6a(val, g.answers);
    return (
      <li key={g.id}>
        <span>
          {g.id}. {g.before}
