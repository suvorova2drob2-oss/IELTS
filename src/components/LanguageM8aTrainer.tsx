...
import {
  ...
  LANG_M8A_STEPS,
  languageM8a,
} from "../data/languageM8a";

const data = languageM8a;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, LANG_M8A_STEPS.length - 1));
}

export function LanguageM8aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LANG_M8A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
