...
import {
  ...
  LANG_M9B_STEPS,
  languageM9b,
} from "../data/languageM9b";

const data = languageM9b;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, LANG_M9B_STEPS.length - 1));
}

export function LanguageM9bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LANG_M9B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
