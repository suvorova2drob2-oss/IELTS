...
import {
  ...
  SPEAK_M10A_STEPS,
  speakingM10a,
} from "../data/speakingM10a";

const data = speakingM10a;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, SPEAK_M10A_STEPS.length - 1));
}

export function SpeakingM10aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= SPEAK_M10A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
