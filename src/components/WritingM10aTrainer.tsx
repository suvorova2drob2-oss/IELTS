...
import {
  ...
  WRITE_M10A_STEPS,
  writingM10a,
} from "../data/writingM10a";
import { WritingComposePanel } from "./WritingComposePanel";

const data = writingM10a;
const DRAFT_KEY = "ielts-writing-M10a-draft";
const PLAN_KEY = "ielts-writing-M10a-plan";

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, WRITE_M10A_STEPS.length - 1));
}
...
export function WritingM10aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= WRITE_M10A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
