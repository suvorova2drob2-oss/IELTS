...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, READ_M8A_STEPS.length - 1));
}

export function ReadingM8aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const examScore = allExamGaps.filter((g) =>
    checkReadM8a(gaps[g.id] ?? "", g.answers),
  ).length;
  ...
  const goNext = () => {
    ...
    if (step >= READ_M8A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
  ...
  const gapRow = (
    ...
    const val = gaps[g.id] ?? "";
    const ok = checkReadM8a(val, g.answers);
    return (
      <li key={g.id}>
        <span>
          {g.id}. {g.before}
