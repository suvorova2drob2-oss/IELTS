...
import {
  ...
  WRITE_M9A_STEPS,
  writingM9a,
} from "../data/writingM9a";
import { WordCountMeter, countWords } from "./WordCountMeter";

const data = writingM9a;
const DRAFT_KEY = "ielts-writing-M9a-draft";
const PLAN_KEY = "ielts-writing-M9a-plan";

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, WRITE_M9A_STEPS.length - 1));
}
...
export function WritingM9aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= WRITE_M9A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
    ...
    <div className="app-shell reading-flow reading-flow--viewport write-m3a">
      ...
      {step === 3 && (
        <section className="read-m3__panel">
          ...
          />
          <WordCountMeter words={countWords(draft)} minWords={150} />
          <button
            type="button"
            className="btn-secondary"
