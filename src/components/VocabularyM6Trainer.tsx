...
import {
  ...
  VOCAB_M6_STEPS,
  vocabularyM6,
} from "../data/vocabularyM6";

const data = vocabularyM6;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, VOCAB_M6_STEPS.length - 1));
}

export function VocabularyM6Trainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= VOCAB_M6_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
