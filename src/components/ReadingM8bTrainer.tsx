...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, READ_M8B_STEPS.length - 1));
}

export function ReadingM8bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const examScore = data.exam.gaps.filter((g) =>
    checkReadM8b(gaps[g.id] ?? "", g.answers),
  ).length;
  ...
  const goNext = () => {
    ...
    if (step >= READ_M8B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
          ...
          <aside className="read-m3__side read-m3__side--exam">
            ...
            <ul className="read-m3__para-slots">
              {data.exam.gaps.map((g) => {
                const val = gaps[g.id] ?? "";
                const ok = checkReadM8b(val, g.answers);
                return (
                  <li key={g.id}>
                    <span>
                      {g.id}. {g.before}
