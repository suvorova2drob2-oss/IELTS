...
import {
  ...
  LANG_M6A_STEPS,
  languageM6a,
} from "../data/languageM6a";

const data = languageM6a;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, LANG_M6A_STEPS.length - 1));
}

export function LanguageM6aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LANG_M6A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
