...
import {
  ...
  LISTEN_M9B_STEPS,
  listeningM9b,
} from "../data/listeningM9b";

const data = listeningM9b;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, LISTEN_M9B_STEPS.length - 1));
}

export function ListeningM9bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LISTEN_M9B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
        ...
        <section className="read-m3__panel">
          ...
          <ul className="read-m3__para-slots listen-m3b__grid">
            {data.exam4.gaps.map((g) => {
              const val = gaps[g.id] ?? "";
              const ok = checkListenM9b(val, g.answers);
              return (
                <li key={g.id}>
                  {g.id}. {g.before}
                  <button
