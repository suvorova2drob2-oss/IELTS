...
function clampStep(n: number | undefined): number {
  ...
  return Math.max(0, Math.min(n, LISTEN_M6A_STEPS.length - 1));
}

export function ListeningM6aTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LISTEN_M6A_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
        ...
        <section className="read-m3__panel">
          ...
          <ul className="read-m3__para-slots">
            {data.prep3.gaps3b.gaps.map((g) => {
              const val = gaps[g.id] ?? "";
              const ok = checkListenM6a(val, g.answers);
              return (
                <li key={g.id}>
                  {g.id}. {g.before}
                  <button
