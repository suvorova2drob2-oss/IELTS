...
import {
  ...
  READ_M9B_STEPS,
  readingM9b,
} from "../data/readingM9b";
import { ExpertDiscussPanel } from "./ExpertDiscussPanel";

const data = readingM9b;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, READ_M9B_STEPS.length - 1));
}

export function ReadingM9bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const examScore = data.exam.gaps.filter((g) =>
    checkReadM9b(gaps[g.id] ?? "", g.answers),
  ).length;
  ...
  const goNext = () => {
    ...
    if (step >= READ_M9B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
          ...
          <aside className="read-m3__side read-m3__side--exam">
            ...
            <ul className="read-m3__para-slots">
              {data.exam.gaps.map((g) => {
                const val = gaps[g.id] ?? "";
                const ok = checkReadM9b(val, g.answers);
                return (
                  <li key={g.id}>
                    <span>
                      {g.id}. {g.before}
