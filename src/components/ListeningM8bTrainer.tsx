...
import {
  ...
  LISTEN_M8B_STEPS,
  listeningM8b,
} from "../data/listeningM8b";

const data = listeningM8b;

function clampStep(n: number | undefined): number {
  if (n == null || Number.isNaN(n)) return 0;
  return Math.max(0, Math.min(n, LISTEN_M8B_STEPS.length - 1));
}

export function ListeningM8bTrainer({
  onBack,
  restart,
  initialStep,
}: {
  ...
  const goNext = () => {
    ...
    if (step >= LISTEN_M8B_STEPS.length - 1) {
      onBack?.();
      return;
    }
    setChecked(false);
        ...
        <section className="read-m3__panel">
          ...
          <ul className="read-m3__para-slots listen-m3b__grid">
            {data.exam4.gaps.map((g) => {
              const val = gaps[g.id] ?? "";
              const ok = checkListenM8b(val, g.answers);
              return (
                <li key={g.id}>
                  {g.id}. {g.before}
                  <button
