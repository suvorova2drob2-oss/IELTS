...
export function checkListenM8a(input: string, accepted: string[]): boolean {
  const n = normalize(input);
  if (!n) return false;
  return accepted.some((a) => normalize(a) === n);
}

export const listeningM8a = {
  id: "listening-m8a-flow",
  bookPages: "p. 124 in your coursebook",
  sectionTitle: "Listening · Section 2 (lines of argument)",
  noAudioNote:
    "Аудио пока нет — можно пройти подготовку и задания. Ответы по Teacher’s Book / script 8.x, когда появятся треки.",
  before: {
    badge: "1",
  ...
  exam4: {
    badge: "3a",
    strategies: "TEST STRATEGIES pages 168–169",
    instruction:
      "Section 2 matching + table completion (no audio yet). TB keys: 1 D  2 B  3 E  4 C  5 Easy to use  6 temporary  7 close friends  8 web version  9 teens  10 demographics",
    gaps: [
      {
        id: 1,
