...
export function checkReadM8b(input: string, accepted: string[]): boolean {
  const n = normalize(input);
  if (!n) return false;
  return accepted.some((a) => normalize(a) === n);
}

export const readingM8b = {
  id: "reading-m8b-flow",
  bookPages: "pp. 130–131 in your coursebook",
  ...
  exam: {
    ...
    instruction:
      "Complete the notes / flow chart. Choose NO MORE THAN THREE WORDS from the passage for each answer. Check spelling and word limits. TB: 1 No  2 Yes  3 No  4 Not given  5 Yes  6/7 B, D.",
    gaps: [
      {
        id: 1,
        before: "Identify ",
